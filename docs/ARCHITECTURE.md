...
| D5 | **Geocoding** | ✅ **OpenStreetMap Nominatim** for search, with the 22-location catalogue as an offline fallback, and `tz-lookup` for the IANA zone | No API key and no billing account, which is what made it the one that could ship. The cost is its usage policy, which limits the *application* to one request per second — so the throttle is a shared reservation in Postgres rather than a per-process queue, and ODbL attribution is carried as data on the provider so swapping the vendor swaps the credit. Costs of changing: `GeocodingProvider` is a two-method interface, `CompositeGeocoder` already merges two implementations, and `GEOCODER_ENDPOINT` / `GEOCODER_UNTHROTTLED` already redirect it at a self-hosted instance — so a paid provider is an added file, not a rewrite. The catalogue is kept rather than deleted because it carries **elevation** — 754 m at Jerusalem moves sunset by minutes — and because a geocoder outage must not block the location step. The zone is derived from the confirmed coordinates by an offline shapefile lookup, never from the browser and never accepted from the client. |
...
| Rate limiting of what *we send* to a third party | `db/outbound-throttle.ts` | A shared **reservation**, not a refusal: each caller takes the next send slot and pushes the marker on. OpenStreetMap's policy bounds the application at one request per second, which a per-process queue cannot honour on a platform that runs many processes |
